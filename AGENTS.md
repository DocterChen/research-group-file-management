...
- 全程优先遵循 `awesome-code` 的多代理规范：先理解任务，再按需拆分，不要跳过验证。
...
- 如果存在疑问、实现风险或更稳妥更清晰的方案，Codex 应在不违反隐私、权限和数据边界的前提下自主选择最优方案并直接落地，不反复向用户确认。
- 最终交付前必须完成可运行验证，并确保程序可正常执行。
- 当用户要求“自己决定，不要问我”时，优先采用最保守且可验证的实现，不因局部不确定而中断任务。
