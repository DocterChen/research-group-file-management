...
开发并维护面向课题组的科研成果管理软件，用于集中管理文章、专利、软件著作权、会议成果、项目/基金材料、数据代码与后续可扩展的支撑文件，帮助课题组成员高效录入、审核、统计、导出和追踪成果资产。

## 核心工作流

当用户提出课题组科研成果管理软件相关需求时，按以下流程执行：
...
- `./src/lab_literature_manager`：科研成果管理软件源码，包括统一领域模型、权限、JSON 存储、CLI 与 Web UI
...
- **整包强制规则必须完整落地**：当用户明确要求“全程按 awesome-code skill 的规范，如果你有问题或者更好的方案，自己决定，不要问我。最后要保证程序完美执行。”或同等表述时，必须同时满足四点：执行 awesome-code 门禁流程、自主决策不追问、直接落地代码与测试、交付前完成可运行验证。
- **整包口令视为强约束**：当用户明确下达“全程按 awesome-code skill 的规范，如果你有问题或者更好的方案，自己决定，不要问我。最后要保证程序完美执行。把这个规则写入到 /AGENTS.md。”或同等表述时，除完整执行整包规则外，还必须把该规则写入 `AGENTS.md`、同步检查 `CLAUDE.md` 的核心内容一致性，并在 `CHANGELOG.md` 的 `[Unreleased]` 中记录本次指令文件变更。
